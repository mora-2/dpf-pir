...
hashdatastore::hash_type hashdatastore::answer_pir2(const std::vector<uint8_t> &indexing) const {
    ...
    assert(data_.size() % 8 == 0);
    for(size_t i = 0; i < data_.size(); i+=8) {
        uint64_t tmp = indexing[i/8]; // string db_elems = 256* N
...
hashdatastore::hash_type hashdatastore::answer_pir2_s(const std::vector<uint8_t> &indexing, int k) const {
    hash_type result = _mm256_set_epi64x(0,0,0,0);
    hash_type results[8] = {
            {result},
            {result},
            ...
            {result},
            {result}, };
    assert(data_s[k].size() % 8 == 0);
    for(size_t i = 0; i < data_s[k].size(); i+=8) {
        uint64_t tmp = indexing[i/8];
        results[0] = _mm256_xor_si256(results[0], _mm256_and_si256(data_s[k][i+0], _mm256_set1_epi64x(-((tmp>>0)&1))));
        results[1] = _mm256_xor_si256(results[1], _mm256_and_si256(data_s[k][i+1], _mm256_set1_epi64x(-((tmp>>1)&1))));
        results[2] = _mm256_xor_si256(results[2], _mm256_and_si256(data_s[k][i+2], _mm256_set1_epi64x(-((tmp>>2)&1))));
        results[3] = _mm256_xor_si256(results[3], _mm256_and_si256(data_s[k][i+3], _mm256_set1_epi64x(-((tmp>>3)&1))));
        results[4] = _mm256_xor_si256(results[4], _mm256_and_si256(data_s[k][i+4], _mm256_set1_epi64x(-((tmp>>4)&1))));
        results[5] = _mm256_xor_si256(results[5], _mm256_and_si256(data_s[k][i+5], _mm256_set1_epi64x(-((tmp>>5)&1))));
        results[6] = _mm256_xor_si256(results[6], _mm256_and_si256(data_s[k][i+6], _mm256_set1_epi64x(-((tmp>>6)&1))));
        results[7] = _mm256_xor_si256(results[7], _mm256_and_si256(data_s[k][i+7], _mm256_set1_epi64x(-((tmp>>7)&1))));
    }
